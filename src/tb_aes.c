...
// The least significant tb_byte of the word is rotated to the end.
...
// first nibble in the first index (row) and the second nibble in the 
// second index (column).
static const tb_byte aes_sbox[16][16] = {
...
static const tb_byte aes_invsbox[16][16] = {
...
// calculations.This table is only used by the (Inv)MixColumns steps.
// USAGE: The second index (column) is the coefficient of multiplication. 
// Only 7 different coefficients are used: 0x01, 0x02, 0x03, 0x09, 0x0b, 0x0d, 
// 0x0e, but multiplication by 1 is negligible leaving only 6 coefficients. 
// Each column of the table is devoted to one of these coefficients, in the 
// ascending order of value, from values 0x00 to 0xFF.
static const tb_byte gf_mul[256][6] = {
...
tb_uint32 SubWord(tb_uint32 word)
{
	tb_uint32 result;

	result = (tb_int32)aes_sbox[(word >> 4) & 0x0000000F][word & 0x0000000F];
	result += (tb_int32)aes_sbox[(word >> 12) & 
		0x0000000F][(word >> 8) & 0x0000000F] << 8;

	result += (tb_int32)aes_sbox[(word >> 20) & 
		0x0000000F][(word >> 16) & 0x0000000F] << 16;

	result += (tb_int32)aes_sbox[(word >> 28) & 
		0x0000000F][(word >> 24) & 0x0000000F] << 24;
...
void AddRoundKey(tb_byte state[][4], const tb_uint32 w[])
{
	tb_byte subkey[4];
...
void InvSubBytes(tb_byte state[][4])
{
...
void ShiftRows(tb_byte state[][4])
{
	tb_int32 t;
...
void InvShiftRows(tb_byte state[][4])
{
	tb_int32 t;
...
// multiplication in a Galios Field 2^8. All multiplication is pre-computed in 
// a table. Addition is equivilent to XOR. (Must always make a copy of the 
// column as the original values will be destoyed.)
void MixColumns(tb_byte state[][4])
{
	tb_byte col[4];
...
void InvMixColumns(tb_byte state[][4])
{
	tb_byte col[4];
...
tb_int32 TB_API tb_aes_key_setup(const tb_byte *key, tb_uint32 *w, 
	tb_int32 keysize)
{
	tb_int32 Nb=4, Nr, Nk, idx;
	tb_uint32 temp, Rcon[] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 
...
void TB_API tb_aes_encrypt(const tb_byte *in, tb_byte *out, 
	const tb_uint32 *key, tb_int32 keysize)
{
	tb_byte state[4][4];

	// Copy input array (should be 16 bytes long) to a matrix (sequential 
	// bytes are ordered by row, not col) called "state" for processing.
...
void TB_API tb_aes_decrypt(const tb_byte *in, tb_byte *out, 
	const tb_uint32 *key, tb_int32 keysize)
{
	tb_byte state[4][4];
...
void xor_buf(const tb_byte in[], tb_byte out[], size_t len)
{
	size_t idx;
...
tb_int32 TB_API tb_aes_encrypt_cbc(const tb_byte *in, size_t in_len, 
	tb_byte *out, const tb_uint32 *key, tb_int32 keysize, const tb_byte *iv)
{
	...
	memcpy(iv_buf, iv, TB_AES_BLOCK_SIZE);
	for (idx = 0; idx < blocks; idx++)
	{
		memcpy(buf_in, &in[idx * TB_AES_BLOCK_SIZE], TB_AES_BLOCK_SIZE);
		xor_buf(iv_buf, buf_in, TB_AES_BLOCK_SIZE);
		tb_aes_encrypt(buf_in, buf_out, key, keysize);
		memcpy(&out[idx * TB_AES_BLOCK_SIZE], buf_out, TB_AES_BLOCK_SIZE);
		memcpy(iv_buf, buf_out, TB_AES_BLOCK_SIZE);
	}
	...
	tb_byte *out, const tb_uint32 *key, tb_int32 keysize, const tb_byte *iv)
{
	...
	{
		memcpy(buf_in, &in[idx * TB_AES_BLOCK_SIZE], TB_AES_BLOCK_SIZE);
		tb_aes_decrypt(buf_in, buf_out, key, keysize);
		xor_buf(iv_buf, buf_out, TB_AES_BLOCK_SIZE);
		memcpy(&out[idx * TB_AES_BLOCK_SIZE], buf_out, TB_AES_BLOCK_SIZE);
		memcpy(iv_buf, buf_in, TB_AES_BLOCK_SIZE);
	}

	return 0;
}

tb_int32 TB_API tb_aes_encrypt_cbc_mac(const tb_byte *in, size_t in_len, 
	tb_byte *out, const tb_uint32 *key, tb_int32 keysize, const tb_byte *iv)
{
	tb_byte buf_in[TB_AES_BLOCK_SIZE];
	...
	memcpy(out, buf_out, TB_AES_BLOCK_SIZE);   // Only output the last block.
...
//////////////////////////////////////////////////////////////////////////
// AES - CTR
void TB_API tb_aes_increment_iv(tb_byte *iv, tb_int32 counter_size)
{
	tb_int32 idx;

	// Use counter_size bytes at the end of the IV as the big-endian integer 
	// to increment.
	for (idx = TB_AES_BLOCK_SIZE - 1; idx >= TB_AES_BLOCK_SIZE - 
		counter_size; idx--)
	{
		iv[idx]++;

		if (iv[idx] != 0 || idx == TB_AES_BLOCK_SIZE - counter_size)
			break;
	}
}
...
void TB_API tb_aes_encrypt_ctr(const tb_byte *in, size_t in_len, 
	tb_byte *out, const tb_uint32 *key, tb_int32 keysize, const tb_byte *iv)
{
	size_t idx = 0, last_block_length;
	tb_byte iv_buf[TB_AES_BLOCK_SIZE], out_buf[TB_AES_BLOCK_SIZE];

	if (in != out)
		memcpy(out, in, in_len);

	memcpy(iv_buf, iv, TB_AES_BLOCK_SIZE);
	last_block_length = in_len - TB_AES_BLOCK_SIZE;

	if (in_len > TB_AES_BLOCK_SIZE)
	{
		for (idx = 0; idx < last_block_length; idx += TB_AES_BLOCK_SIZE)
		{
			tb_aes_encrypt(iv_buf, out_buf, key, keysize);
			xor_buf(out_buf, &out[idx], TB_AES_BLOCK_SIZE);
			tb_aes_increment_iv(iv_buf, TB_AES_BLOCK_SIZE);
		}
	}
	tb_aes_encrypt(iv_buf, out_buf, key, keysize);
...
void TB_API tb_aes_decrypt_ctr(const tb_byte *in, size_t in_len, 
	tb_byte *out, const tb_uint32 *key, tb_int32 keysize, const tb_byte *iv)
{
	// CTR encryption is its own inverse function.
	tb_aes_encrypt_ctr(in, in_len, out, key, keysize, iv);
}

//////////////////////////////////////////////////////////////////////////
// AES - CCM
// Creates the first counter block. First tb_byte is flags, then the nonce, 
// then the incremented part.
void ccm_prepare_first_ctr_blk(tb_byte counter[], const tb_byte nonce[], 
	tb_int32 nonce_len, tb_int32 payload_len_store_size)
{
	memset(counter, 0, TB_AES_BLOCK_SIZE);
	counter[0] = (payload_len_store_size - 1) & 0x07;
	memcpy(&counter[1], nonce, nonce_len);
}

void ccm_prepare_first_format_blk(tb_byte buf[], tb_int32 assoc_len, 
	tb_int32 payload_len, tb_int32 payload_len_store_size, 
	tb_int32 mac_len, const tb_byte nonce[], tb_int32 nonce_len)
{
	// Set the flags for the first tb_byte of the first block.
	...
	// Format the rest of the first block, storing the nonce and the size of 
	// the payload.
	memcpy(&buf[1], nonce, nonce_len);
	memset(&buf[1 + nonce_len], 0, TB_AES_BLOCK_SIZE - 1 - nonce_len);
	buf[15] = payload_len & 0x000000FF;
	buf[14] = (payload_len >> 8) & 0x000000FF;
}

void ccm_format_assoc_data(tb_byte buf[], tb_int32 *end_of_buf, 
	const tb_byte assoc[], tb_int32 assoc_len)
{
	tb_int32 pad;
	...
	*end_of_buf += assoc_len;
	pad = TB_AES_BLOCK_SIZE - (*end_of_buf % TB_AES_BLOCK_SIZE); /*BUG?*/
	memset(&buf[*end_of_buf], 0, pad);
	*end_of_buf += pad;
}

void ccm_format_payload_data(tb_byte buf[], tb_int32 *end_of_buf, 
	const tb_byte payload[], tb_int32 payload_len)
{
	tb_int32 pad;

	memcpy(&buf[*end_of_buf], payload, payload_len);
	*end_of_buf += payload_len;
	pad = *end_of_buf % TB_AES_BLOCK_SIZE;

	if (pad != 0)
		pad = TB_AES_BLOCK_SIZE - pad;
...
tb_int32 TB_API tb_aes_encrypt_ccm(const tb_byte *payload, 
	tb_uint32 payload_len, const tb_byte *assoc, tb_uint16 assoc_len,
	const tb_byte *nonce, tb_uint16 nonce_len, tb_byte *out, 
	tb_uint32 *out_len, tb_uint32 mac_len, 
	const tb_byte *key_str, tb_int32 keysize)
{
	tb_byte temp_iv[TB_AES_BLOCK_SIZE];
	tb_byte counter[TB_AES_BLOCK_SIZE], mac[16], *buf;
	tb_int32 end_of_buf, payload_len_store_size;
	tb_uint32 key[60];
	...
	/*Round both payload and associated data up a block size and add an extra block.*/
	buf = (tb_byte*)malloc(payload_len + assoc_len + 48);
	if (!buf)
		return TB_ERR_NOMEM;

	// Prepare the key for usage.
	tb_aes_key_setup(key_str, key, keysize);

	// Format the first block of the formatted data.
	payload_len_store_size = TB_AES_BLOCK_SIZE - 1 - nonce_len;
	ccm_prepare_first_format_blk(buf, assoc_len, payload_len, 
		payload_len_store_size, mac_len, nonce, nonce_len);

	end_of_buf = TB_AES_BLOCK_SIZE;
	...
	memset(temp_iv, 0, TB_AES_BLOCK_SIZE);
	tb_aes_encrypt_cbc_mac(buf, end_of_buf, mac, key, keysize, temp_iv);
	...
	memcpy(temp_iv, counter, TB_AES_BLOCK_SIZE);

	// Last argument is the tb_byte size of the counting portion of the 
	// counter block. /*BUG?*/
	tb_aes_increment_iv(temp_iv, TB_AES_BLOCK_SIZE - 1 - mac_len);
	tb_aes_encrypt_ctr(out, payload_len, out, key, keysize, temp_iv);

	// Encrypt the MAC with CTR mode with a counter starting at 0.
	tb_aes_encrypt_ctr(&out[payload_len], mac_len, &out[payload_len], key, 
		keysize, counter);
...
tb_int32 TB_API tb_aes_decrypt_ccm(const tb_byte *ciphertext, 
	tb_uint32 ciphertext_len, const tb_byte *assoc, tb_uint16 assoc_len,
	const tb_byte *nonce, tb_uint16 nonce_len, tb_byte *plaintext, 
	tb_uint32 *plaintext_len, tb_uint32 mac_len, tb_int32 *mac_auth, 
	const tb_byte *key_str, tb_int32 keysize)
{
	tb_byte temp_iv[TB_AES_BLOCK_SIZE];
	tb_byte counter[TB_AES_BLOCK_SIZE], mac[16], mac_buf[16], *buf;
	tb_int32 end_of_buf, plaintext_len_store_size;
	tb_uint32 key[60];
	...
	buf = (tb_byte*)malloc(assoc_len + ciphertext_len + 48);
	if (!buf)
		return TB_ERR_NOMEM;
	...
	*plaintext_len = ciphertext_len - mac_len;
	plaintext_len_store_size = TB_AES_BLOCK_SIZE - 1 - nonce_len;
	...
	// (TB_AES_BLOCK_SIZE - 1 - mac_len) is the tb_byte size of the counting 
	// portion of the counter block.
	tb_aes_increment_iv(temp_iv, TB_AES_BLOCK_SIZE - 1 - mac_len);
	tb_aes_decrypt_ctr(plaintext, *plaintext_len, plaintext, key, keysize, 
		temp_iv);
	...
	if (mac_auth != NULL)
	{
		...
		tb_aes_decrypt_ctr(mac, mac_len, mac, key, keysize, counter);

		// Format the first block of the formatted data.
		plaintext_len_store_size = TB_AES_BLOCK_SIZE - 1 - nonce_len;
		ccm_prepare_first_format_blk(buf, assoc_len, *plaintext_len, 
			plaintext_len_store_size, mac_len, nonce, nonce_len);
		end_of_buf = TB_AES_BLOCK_SIZE;
		...
		// Perform the CBC operation with an IV of zeros on the formatted 
		// buffer to calculate the MAC.
		memset(temp_iv, 0, TB_AES_BLOCK_SIZE);
		tb_aes_encrypt_cbc_mac(buf, end_of_buf, mac_buf, key, 
			keysize, temp_iv);

		// Compare the calculated MAC against the MAC embedded in the 
		// ciphertext to see if they are the same.
